...
export default function EditarProductoPage() {
  ...
  useEffect(() => {
    const loadProduct = () => {
      if (product) {
        // Usar imágenes del producto directamente
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FDF9F3]">
        <div className="text-center">
          <p className="font-cormorant text-[#6B4423]">Producto no encontrado</p>
          <Link href="/admin" className="text-[#6B4423] underline">
            Volver al panel
          </Link>
        </div>
      </div>
      ...
      <header className="bg-[#6B4423] text-[#F6D3B3]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-cinzel text-xl tracking-wide">Editar Producto</h1>
              <p className="font-cormorant text-sm text-[#F6D3B3]/70">{product.name}</p>
            </div>
          </div>
        </div>
      </header>
